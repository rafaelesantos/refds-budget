...
public final class TransactionsMiddleware<State>: RefdsReduxMiddlewareProtocol {
    ...
    @RefdsInjection private var transactionRowViewDataAdapter: TransactionItemViewDataAdapterProtocol
    ...
    private func getTransactionsAdapted(
        ...
        let categories = Dictionary(
            grouping: categoryRepository.getAllCategories(),
            by: { $0.id }
        )
        ...
        let transactionsAdapted: [TransactionItemViewDataProtocol] = transactions.compactMap {
            guard let category = categories[$0.category]?.first else { return nil }
    ...
    private func fetchTransactionForEdit(
        ...
        let categories: [CategoryItemViewDataProtocol] = categoryRepository.getCategories(from: transaction.date.date).compactMap {

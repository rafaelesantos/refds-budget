...
public final class FileFactory {
    ...
    public func importData(from file: FileModelProtocol, on url: URL) throws {
        try database.viewContext.performAndWait {
            ...
            let categoriesModel = categoryRepository.getAllCategories()
            newFile.categories = file.categories.map { model in
                var model = model
                let myCategory = categoriesModel.first(where: { $0.name == model.name })
                model.id = myCategory?.id ?? model.id
                return model
            }
            ...
            newFile.budgets = file.budgets.map { model in
                var model = model
                let fileCategory = file.categories.first(where: { $0.id == model.category })
                let myCategory = categories.first(where: { $0.name == fileCategory?.name })
                model.category = myCategory?.id ?? model.category
                return model
            }
            
            newFile.budgets.forEach {
                budgets += [$0.getEntity(for: self.database.viewContext)]
            }
            
            newFile.transactions = file.transactions.map { model in
                var model = model
                let fileCategory = file.categories.first(where: { $0.id == model.category })
                let myCategory = categories.first(where: { $0.name == fileCategory?.name })
                model.category = myCategory?.id ?? model.category
